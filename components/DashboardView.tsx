...
const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode; gradient: string; }> = ({ title, value, icon, gradient }) => (
  <div className={`relative p-6 rounded-xl shadow-md text-white overflow-hidden ${gradient}`}>
...
const ActionButton: React.FC<{ title: string; icon: React.ReactNode; onClick: () => void }> = ({ title, icon, onClick }) => (
  <button onClick={onClick} className="flex flex-col items-center justify-center p-6 bg-gradient-to-br from-teal-500 to-primary-500 hover:from-teal-600 hover:to-primary-600 text-white rounded-xl shadow-lg transition-transform transform hover:-translate-y-1 w-full text-center">
    {icon}
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Current Balance" value={`${balance.toFixed(2)} Rs`} icon={<WalletIcon className="w-24 h-24" />} gradient="bg-gradient-to-br from-teal-500 to-primary-600" />
        <StatCard title="Tasks Completed" value={tasksCompleted} icon={<EarnIcon className="w-24 h-24" />} gradient="bg-gradient-to-br from-amber-500 to-amber-600" />
        <StatCard title="Total Referrals" value={referrals} icon={<CreateTaskIcon className="w-24 h-24" />} gradient="bg-gradient-to-br from-pink-500 to-red-500" />
      </div>
