...
const Header: React.FC<HeaderProps> = ({ username, setIsSidebarOpen, setActiveView }) => {
  ...
  return (
    <header className="bg-white/80 backdrop-blur-md sticky top-0 z-20 shadow-sm border-b border-neutral-200">
      {isBannerVisible && (
        <div className="bg-secondary-700 text-white text-sm font-medium p-2 text-center relative">
          <span 
      ...
      <div className="flex items-center justify-between p-3">
        <div className="flex items-center gap-2">
           <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-gray-600">
            ...
            <span className="font-bold text-lg text-secondary-800 tracking-tight">TaskMint</span>
        </div>
        <div className="flex items-center gap-3">
            <button onClick={() => setActiveView('SPIN_WHEEL')} className="text-secondary-600 p-2 rounded-full hover:bg-secondary-50 transition-colors">
                <GiftIcon className="w-6 h-6" />
            </button>
            <button onClick={() => setActiveView('PROFILE_SETTINGS')}>
                <img src={`https://api.dicebear.com/8.x/initials/svg?seed=${username}`} alt="User Profile" className="w-9 h-9 rounded-full border-2 border-primary-400 p-0.5" />
            </button>
        </div>
      </div>
    </header>
