...
import { ClipboardListIcon, CheckCircleIcon, JazzCashIcon, EasyPaisaIcon, PlusCircleIcon, WalletIcon, ArrowRight } from './icons';
...
const StatusBadge: React.FC<{ status?: string }> = ({ status }) => {
    ...
    return (
        <span className={`px-2.5 py-0.5 text-[10px] font-bold rounded-md border uppercase tracking-wide ${styles}`}>
            {status}
        </span>
    );
}
...
const DepositView: React.FC<DepositViewProps> = ({ onDeposit, transactions }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-24 px-2 animate-fade-in">
      
      <div className="text-center mb-8">
          <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">Add Funds</h2>
          <p className="text-slate-500 mt-2">Top up your wallet to create tasks or upgrade plans.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                    {(Object.keys(accountDetails) as DepositMethod[]).map(methodKey => (
                         ...
                         >
                             ...
                             <div className="scale-110">{accountDetails[methodKey].icon}</div>
            ...
            {/* Submission Form */}
            <div className="bg-white p-6 rounded-3xl shadow-subtle border border-gray-100 relative overflow-hidden">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div>
                        ...
                        <div className="relative">
                            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} className="block w-full pl-4 pr-12 py-3 border-none bg-slate-50 rounded-xl font-bold text-slate-900 focus:ring-2 focus:ring-amber-500 transition-all" placeholder="1000" required />
                    ...
                    <div>
                        ...
                        <input type="text" value={transactionId} onChange={e => setTransactionId(e.target.value)} className="block w-full px-4 py-3 border-none bg-slate-50 rounded-xl font-bold text-slate-900 focus:ring-2 focus:ring-amber-500 transition-all placeholder-slate-300" placeholder="e.g. 1234567890" required />
                    </div>
                    ...
                    {message && <p className={`text-center text-xs font-bold p-2 rounded-lg ${message.includes('submitted') ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>{message}</p>}
                </form>
            </div>
        </div>

        {/* Right Column: History Ledger */}
        <div className="bg-white p-0 sm:p-6 rounded-3xl shadow-subtle border border-gray-100 flex flex-col h-full min-h-[400px]">
                ...
                {depositHistory.length > 0 ? (
                    <div className="space-y-3">
                        {depositHistory.map((tx, i) => (
                            <div key={tx.id} className="group bg-slate-50 hover:bg-white p-4 rounded-2xl border border-gray-100 hover:border-amber-200 hover:shadow-md transition-all duration-200 flex justify-between items-center animate-fade-in-up" style={{animationDelay: `${i * 50}ms`}}>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wide">
                                            {tx.date?.toDate ? tx.date.toDate().toLocaleDateString() : 'Date pending'}
                                        </p>
                                    </div>
                                </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-gray-400 py-12">
                        <WalletIcon className="w-12 h-12 mb-3 opacity-20" />
